...
attr_nodelist2<-merge(nodelist2,samb_usfws_2,by = "name",all.x = TRUE)
...
inDegree<-igraph::degree(g2,mode="in")
outDegree<-igraph::degree(g2,mode="out")
TotalDegree<-as.data.frame(igraph::degree(g2))
betweenness<-igraph::betweenness(g2, v = V(g2), directed = TRUE, weights = NULL,
  nobigint = TRUE, normalized = TRUE)
inCloseness<-igraph::closeness(g2, vids = V(g2), mode = c("in"),normalized = TRUE)
outCloseness<-igraph::closeness(g2, vids = V(g2), mode = c("out"),normalized = TRUE)
#eigen<-evcent(m1,g=1,gmode = "digraph",rescale=TRUE, ignore.eval=TRUE)
#removed eigen from this DF_Centrality
DF_Centrality2<-as.data.frame(cbind(rownames(TotalDegree),inDegree,outDegree,TotalDegree,betweenness,inCloseness,outCloseness))
...
visNetwork(sort(gvis2_science$nodes),gvis2_science$edges,main="Science Applications Connections",width = "100%",height = "850px") %>%
  visLayout(randomSeed = 15) %>%
  visEdges(arrows = 'to', smooth =T)  %>% 
  visPhysics(stabilization = FALSE)  %>%
  visIgraphLayout(smooth=FALSE,physics=FALSE) %>%

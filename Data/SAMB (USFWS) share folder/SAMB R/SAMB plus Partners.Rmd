...
V(g3_permit)[V(g3_permit)$organization=="SAMB"]$color <- "black"
...
visNetwork(sort(gvis3_permit$nodes),gvis3_permit$edges,main="Permittting Connections with External Partners",width = "100%",height = "850px") %>%
  visLayout(randomSeed = 15) %>%
  visEdges(arrows = 'to', smooth =T)  %>% 
  visPhysics(stabilization = FALSE)  %>%
  visIgraphLayout(smooth=FALSE,physics=FALSE) %>%
